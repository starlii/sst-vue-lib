...
export default {
      ...
      default: () => {
        return [
          ...
          // },
          {
            ...
            end: getLastYearMountsToToday(new Date().getFullYear(), new Date().getMonth() + 1)
          }
          // {
  ...
  methods: {
    pickerFocus () {
      setTimeout(() => {
        const sstDatePicker = document.querySelector(`.sst-date-picker-${this.cuid}`).querySelectorAll('.el-picker-panel__shortcut')
        const formDate = {
          formDateYear_1: this.formDataSelf.time ? new Date(this.formDataSelf.time[0]).getFullYear() : null,
          formDateYear_2: this.formDataSelf.time ? new Date(this.formDataSelf.time[1]).getFullYear() : null,
          formDateMonth_1: this.formDataSelf.time ? new Date(this.formDataSelf.time[0]).getMonth() : null,
          formDateMonth_2: this.formDataSelf.time ? new Date(this.formDataSelf.time[1]).getMonth() : null,
          formDateDate_1: this.formDataSelf.time ? new Date(this.formDataSelf.time[0]).getDate() : null,
          formDateDate_2: this.formDataSelf.time ? new Date(this.formDataSelf.time[1]).getDate() : null
        }
        sstDatePicker.forEach((item, index) => {
          const shortcut = {
            shortcutsYear_1: new Date(this.pickerOptions.shortcuts[index].startAndEndTime[0]).getFullYear(),
            shortcutsYear_2: new Date(this.pickerOptions.shortcuts[index].startAndEndTime[1]).getFullYear(),
            shortcutsMonth_1: new Date(this.pickerOptions.shortcuts[index].startAndEndTime[0]).getMonth(),
            shortcutsMonth_2: new Date(this.pickerOptions.shortcuts[index].startAndEndTime[1]).getMonth(),
            shortcutsDate_1: new Date(this.pickerOptions.shortcuts[index].startAndEndTime[0]).getDate(),
            shortcutsDate_2: new Date(this.pickerOptions.shortcuts[index].startAndEndTime[1]).getDate()
          }
          item.className = 'el-picker-panel__shortcut'
          if (this.formDataSelf.time) {
            const isEqual = ['Year', 'Month', 'Date'].map(o => {
              return shortcut[`shortcuts${o}_1`] === formDate[`formDate${o}_1`] && shortcut[`shortcuts${o}_2`] === formDate[`formDate${o}_2`]
            }).every(i => i === true)
            if (isEqual) {
