  ...
  <div class="sst-filter" :class="effect === 'dark' ? 'dark' : ''">
    <div>
      ...
      <span class="btn-container">
        <el-button :size="size" v-if="searchShow" type="warning" @click="handleSearch">Search</el-button>
        <el-button :size="size" v-if="resetShow" @click="handleReset">Reset</el-button>
      ...
      >
        <filter-option
          ...
          :type="filterType"
          :selectOptions="selectFilter"
          @cancel="handleCancel"
          @select="handleSelect"
          @cancelAll="handleCancelAll"
          @selectAll="handleSelectAll"
        />
        <el-button :size="size" slot="reference" class="sst-filter__btn-more sst-filter__link">
          More Filter(s)
          <i class="iconfont icon-down1 right"></i>
        </el-button>
      </el-popover>

    </div>
    <div v-if="filterType === 'manager' && selectFilter.length" class="filter-container">
      <el-checkbox-group v-model="selectFilter" @change="handleManageSelect">
        <el-checkbox v-for="item in selectFilter" :label="item" :key="item"></el-checkbox>
      </el-checkbox-group>
    </div>
    <div v-if="filterType === 'detail' && selectFilter.length" class="filter-container">
      <filter-item v-for="filter in selectFilter"
                   :ref="filter"
                   :size="size"
                   :key="filter"
                   :filter="filter"
    ...
    <el-dialog v-if="filterType === 'detail'" title="Device List" :visible.sync="deviceDialogVisible" top="2vh" width="65%">
      <filter-device
        :size="size"
        :deviceDate="filterData.Device || []"
        :deviceBrandList="deviceBrandList"
        :deviceSeriesList="deviceSeriesList"
        :deviceBatchList="deviceBatchList"
    ...
    <el-dialog  v-if="filterType === 'detail'" title="Item Labels" :visible.sync="labelsDialogVisible" width="40%">
      <filter-labels
        :labelsDate="filterData['Item Labels'] || []"
        @closeDialog="handleClose"
      />
    </el-dialog>
  </div>
...
export default {
  ...
  props: {
    ...
    effect: { // bar theme, Accepted Values: light/dark
      type: String,
      default: 'light'
    },
    filterType: {
    ...
    initFilterDate(val, flag = false) {
      val.map(filter => {
        ...
        if (this.filterConfig.timeAndValue.indexOf(filter) >= 0) {
          ...
          if (flag) {
            this.$refs[filter][0].filterDateCopy = { time: null, min: null, max: null }
        ...
        if (this.filterConfig.reOrder.indexOf(filter) >= 0) {
          ...
          if (flag) {
            this.$refs[filter][0].filterDateCopy = { relation: null, min: null, max: null }
            this.$refs[filter][0].clear({ relation: null, min: null, max: null })
          }
        }
        ...
        if (this.filterConfig.selectAndTimeAndValue.indexOf(filter) >= 0) {
          this.filterForm[filter] = { value: [], time: null, min: null, max: null }
          if (flag) {
            this.$refs[filter][0].filterDateCopy = { value: [], time: null, min: null, max: null }
            this.$refs[filter][0].clear({ value: [], time: null, min: null, max: null })
          }
        }
  ...
  created() {
    ...
    const _filterConfig = ['detail', 'device', 'label', 'reOrder', 'time', 'timeAndValue', 'value', 'select', 'selectAndTimeAndValue']
